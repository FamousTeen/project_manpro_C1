...
<div class="container mx-auto py-8 mt-8">
    ...
    <div class="flex justify-center mb-16">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-16 ">
            ...
            <!-- Pengumuman {{ $index}} -->
            <div class="bg-[#f6f1e3] p-6 shadow-lg w-64 mx-8 border border-[#002366] rounded-lg cursor-pointer" onclick="openModal('modal{{$index}}')">
                <div class="flex justify-between items-center">
                    <h2 class="text-lg font-bold mb-2">{{ Carbon::parse($announcement->upload_time)->translatedFormat('l, j F Y') }}</h2>
                ...
                <div class="bg-white p-8 rounded-lg w-1/2 relative" onclick="event.stopPropagation()">
                    ...
                    <form class="max-w" method="post" action="{{ route('announcements.update', ['announcement' => $announcement])}}">
                        ...
                        <div class="mt-6 flex justify-end space-x-4">
                            ...
                            <button type="button" class="bg-[#ae0001] text-white px-4 py-2 rounded" onclick="closeModal('modal{{$index++}}')">Tutup</button>
                        </div>
                    </form>
                </div>
            </div>
