...
@section('content')
<div class="container-fluid w-full m-12 mt-24">
  <!-- Header Section -->
...
<div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 justify-items-center gap-16 m-12 mt-10">
  ...
  $trainings = Training::whereHas('trainingDetails', function ($query) use ($user) {
  ...
  <!-- Modal 1 -->
  <div id="modal{{$training->id}}" class="modal hidden fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center" onclick="closeModal('modal1')">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="text-left">
          <div class="flex items-center">
            <h2 class="text-2xl font-bold ml-2">{{Carbon::parse($training->training_date)->translatedFormat('l, d-M-Y')}}</h2>
          </div>
          <div class="ms-2">
            <p class="mt-2 text-lg">{{Carbon::parse($training->training_date)->translatedFormat('H:i')}} WIB</p>
            <div class="mt-4">
              <p class="font-bold">Contact Person:</p>
              <p>Nama: {{$training->contact_person}}</p>
              <p>No HP: <a style="color: blue; text-decoration: underline; cursor: pointer;" href="https://wa.me/62{{ $training->phone_number }}">{{ $training->phone_number }}</a></p>
